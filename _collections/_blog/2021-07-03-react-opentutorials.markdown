...
props는 사용자가 component를 사용하는데 있어 중요한 것이고,  
state는 props의 값에 따라 내부 구현에 필요한 데이터다.   
 
component의 기본적인 동작을 바꾸려고 사용자에게 제공하는 것이  
...
사용자가 몰라도 되는 component 내부에서 사용되는 것을    
state라고 한다.   
  

component가 좋은 부품이 되기 위해서는   
...
이 객체를 사용하기 위해서는 아래와 같이 App Component를 바꿔준다.  
